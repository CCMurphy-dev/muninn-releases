...
| `series` | Auto | Series metadata array (written by Organizer) |
| `organized_at` | Auto | Timestamp when organized (written by Organizer) |
| `source_path` | Auto | Original DICOM source path (written by Organizer) |

#### Series Metadata (Automatic)

When you use the DICOM Organizer, it automatically writes series metadata to `case_data.json`. This enables fast loading without re-reading DICOM headers on every open.
...
```json
{
  ...
  "series": [
    ...
    {
      "series_uid": "1.2.840.113619.2.55.3.604688120",
      "description": "CT Coronal Reformat",
      "modality": "CT",
      "slice_count": 120,
      "folder_name": "02_CT_Coronal_Reformat",
      "window_width": 400,
      "window_center": 40
    }
  ],
  "organized_at": "2026-02-11T10:30:00Z",
  "source_path": "/Users/export/pacs_dump"
}
```
...
| `description` | Series description from DICOM tags |
| `modality` | CT, MR, XR, etc. |
| `slice_count` | Number of slices in the series |
| `folder_name` | Name of the series folder |
| `window_width` | Default window width from DICOM |
| `window_center` | Default window center from DICOM |
| `rows` | Image rows (pixels) |
| `columns` | Image columns (pixels) |
| `bits_allocated` | Bit depth (8, 12, 16) |
| `study_description` | Study-level description (optional) |

> **Note:** The `series` array is automatically written by the DICOM Organizer and preserved when Case Loader saves teaching metadata. You don't need to create this manually.
...
For richer metadata including series information. The `series` array is automatically generated by the DICOM Organizer:
...
```json
{
  ...
  "series": [
    ...
    {
      "series_uid": "1.2.840.113619.2.55.3.604688121",
      "description": "T2 FLAIR",
      "modality": "MR",
      "slice_count": 32,
      "folder_name": "03_T2_FLAIR",
      "window_width": 1035,
      "window_center": 517
    }
  ],
...
> **Performance:** When `case_data.json` contains the `series` array, Muninn loads cases instantly without scanning DICOM headers. This is especially important for cases on network drives.
